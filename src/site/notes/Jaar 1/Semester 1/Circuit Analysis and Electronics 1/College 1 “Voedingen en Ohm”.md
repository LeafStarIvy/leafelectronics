---
{"dg-publish":true,"permalink":"/jaar-1/semester-1/circuit-analysis-and-electronics-1/college-1-voedingen-en-ohm/","noteIcon":"","created":"2026-09-15T09:42:39.003+02:00","updated":"2026-09-15T11:50:46.395+02:00","dg-note-properties":{}}
---
...
<title>Spanning, Stroom & Weerstand — Interactieve Uitleg</title>
<style>
  :root{
    --bg: #12161c;
    --bg-panel: #1a2029;
  ...
  *{ box-sizing: border-box; }
  ...
  p.lead{ color: var(--text-dim); margin-bottom: 22px; }
  ...
  .formula-box.blue{ border-left-color: var(--blue); color: var(--blue-bright); }
  .formula-box.green{ border-left-color: var(--green); color: var(--green-bright); }
  ...
  .var-key div{ padding: 3px 0; }
  .var-key b{ color: var(--text); font-weight: 600; }
  ...
  .control-row .value{ color: var(--copper-bright); font-weight: 600; }
  .control-row .value.blue{ color: var(--blue-bright); }
  .control-row .value.green{ color: var(--green-bright); }
  ...
  .control.blue input[type="range"]::-webkit-slider-thumb{ background: var(--blue-bright); box-shadow: 0 0 0 1px var(--blue); }
  .control.green input[type="range"]::-webkit-slider-thumb{ background: var(--green-bright); box-shadow: 0 0 0 1px var(--green); }
  ...
  .readout-item.highlight .rvalue{ color: var(--copper-bright); }
  ...
  .callout.warn{ border-left: 3px solid var(--red); }
  .callout.warn .ctitle{ color: var(--red); }
  .callout.correction{ border-left: 3px solid var(--copper); }
  .callout.correction .ctitle{ color: var(--copper-bright); }
...
<div class="wrap">
  ...
  <section class="first">
    ...
    <div class="panel">
      ...
      <svg class="diagram" viewBox="0 0 680 200" id="coulombSvg">
        ...
        <g id="electronDots"></g>
        <text x="340" y="192" text-anchor="middle" fill="#dfe4ea" font-family="monospace" font-size="13" id="coulombCounter">0 × 10¹⁸ elektronen</text>
      </svg>
      <div class="controls">
        <div class="control green">
          <div class="control-row"><span class="label">verzamelde lading</span><span class="value green" id="coulombVal">0.00 C</span></div>
          <input type="range" id="coulombSlider" min="0" max="100" value="0" step="1">
        </div>
      </div>
      <div class="readout">
        <div class="readout-item highlight">
          <div class="rlabel">q (lading)</div>
          <div class="rvalue" id="coulombQOut">0.00 C</div>
        </div>
      </div>
    </div>
  ...
  <section>
    ...
    <div class="panel">
      ...
      <svg class="diagram" viewBox="0 0 680 160" id="deltaSvg">
        ...
        <circle id="deltaStart" cx="120" cy="120" r="6" fill="#5b8dd9"/>
        <circle id="deltaEnd" cx="560" cy="60" r="6" fill="#c9803d"/>
        <line id="deltaBracket" x1="120" y1="120" x2="560" y2="60" stroke="#e6a25c" stroke-width="1.5" stroke-dasharray="4 3"/>
        <text id="deltaLabel" x="340" y="40" text-anchor="middle" fill="#e6a25c" font-family="monospace" font-size="14" font-weight="600">ΔT = 0°C</text>
      </svg>
      <div class="controls">
        <div class="control blue">
          <div class="control-row"><span class="label">begin-temperatuur</span><span class="value blue" id="deltaStartVal">15°C</span></div>
          <input type="range" id="deltaStartSlider" min="0" max="40" value="15" step="1">
        </div>
        <div class="control">
          <div class="control-row"><span class="label">eind-temperatuur</span><span class="value" id="deltaEndVal">22°C</span></div>
          <input type="range" id="deltaEndSlider" min="0" max="40" value="22" step="1">
        </div>
      </div>
    </div>
  ...
  <section>
    ...
    <div class="panel">
      ...
      <svg class="diagram" viewBox="0 0 680 140" id="currentSvg">
        ...
        <g id="chargeDots"></g>
        <text x="340" y="122" text-anchor="middle" fill="#8b93a3" font-family="monospace" font-size="12" id="currentCaption">elektronen bewegen door de draad →</text>
      </svg>
      <div class="controls">
        <div class="control">
          <div class="control-row"><span class="label">stroomsterkte I</span><span class="value" id="currentVal">1.0 A</span></div>
          <input type="range" id="currentSlider" min="0.2" max="4" value="1" step="0.1">
        </div>
      </div>
      <div class="readout">
        <div class="readout-item">
          <div class="rlabel">Δq per seconde</div>
          <div class="rvalue" id="currentQOut">1.0 C/s</div>
        </div>
        <div class="readout-item highlight">
          <div class="rlabel">I</div>
          <div class="rvalue" id="currentIOut">1.0 A</div>
        </div>
      </div>
    </div>
  ...
  <section>
    ...
    <div class="panel">
      ...
      <svg class="diagram" viewBox="0 0 680 220" id="voltageSvg">
        <line x1="60" y1="200" x2="620" y2="200" stroke="#2c3444" stroke-width="2"/>
        <!-- reservoir -->
        <rect id="reservoirTank" x="120" y="40" width="90" height="140" fill="none" stroke="#2c3444" stroke-width="2"/>
        <rect id="reservoirWater" x="122" y="120" width="86" height="58" fill="#5b8dd9" opacity="0.5"/>
        <text x="165" y="30" text-anchor="middle" fill="#8b93a3" font-family="monospace" font-size="11">reservoir A</text>
        <!-- pipe -->
        <path id="voltagePipe" d="M 210 175 Q 350 175 430 175" stroke="#5b8dd9" stroke-width="4" fill="none" opacity="0.7"/>
        <!-- ground level -->
        ...
        <text id="voltageDropLabel" x="330" y="90" text-anchor="middle" fill="#7ba7ea" font-family="monospace" font-size="13" font-weight="600">U = 5 V</text>
      </svg>
      <div class="controls">
        <div class="control blue">
          <div class="control-row"><span class="label">spanning U (hoogteverschil)</span><span class="value blue" id="voltageVal">5 V</span></div>
          <input type="range" id="voltageSlider" min="0" max="12" value="5" step="0.5">
        </div>
      </div>
      <div class="readout">
        <div class="readout-item highlight">
          <div class="rlabel">U</div>
          <div class="rvalue" id="voltageUOut">5 V</div>
        </div>
        <div class="readout-item">
          <div class="rlabel">"drang" om te stromen</div>
          <div class="rvalue" id="voltageDescOut">gemiddeld</div>
        </div>
      </div>
    </div>
  ...
  <section>
    ...
    <div class="panel">
      ...
      <svg class="diagram" viewBox="0 0 680 160" id="ohmSvg">
        <path d="M 40 70 L 260 70" stroke="#2c3444" stroke-width="30" fill="none"/>
        <path id="ohmNeck" d="M 260 70 L 420 70" stroke="#2c3444" stroke-width="30" fill="none"/>
      ...
      <div class="controls">
        <div class="control blue">
          <div class="control-row"><span class="label">spanning U</span><span class="value blue" id="ohmUVal">10 V</span></div>
          <input type="range" id="ohmUSlider" min="1" max="20" value="10" step="0.5">
        </div>
        <div class="control green">
          <div class="control-row"><span class="label">weerstand R</span><span class="value green" id="ohmRVal">100 Ω</span></div>
          <input type="range" id="ohmRSlider" min="10" max="500" value="100" step="10">
        </div>
      </div>
      <div class="readout">
        <div class="readout-item highlight">
          <div class="rlabel">I = U / R</div>
          <div class="rvalue" id="ohmIOut">0.100 A</div>
        </div>
      </div>
    </div>
    ...
    <div class="panel">
      ...
      <svg class="diagram" viewBox="0 0 400 260" id="ohmGraphSvg">
        ...
        <line id="ohmGraphLine" x1="50" y1="220" x2="370" y2="150" stroke="#99c980" stroke-width="2.5"/>
        <circle id="ohmGraphDot" cx="290" cy="180" r="5" fill="#e6a25c"/>
      </svg>
    </div>
  </section>
  ...
  <section>
    ...
    <div class="panel">
      ...
      <svg class="diagram" viewBox="0 0 680 140" id="uabSvg">
        ...
        <path id="uabArrow" d="M 480 60 L 500 70 L 480 80" fill="none" stroke="#e6a25c" stroke-width="2.5"/>
        <text id="uabResultLabel" x="340" y="45" text-anchor="middle" fill="#e6a25c" font-family="monospace" font-size="14" font-weight="600">U_ab = 6 V</text>
      </svg>
      <div class="controls">
        <div class="control blue">
          <div class="control-row"><span class="label">U in punt a</span><span class="value blue" id="uaVal">8 V</span></div>
          <input type="range" id="uaSlider" min="-12" max="12" value="8" step="1">
        </div>
        <div class="control blue">
          <div class="control-row"><span class="label">U in punt b</span><span class="value blue" id="ubVal">2 V</span></div>
          <input type="range" id="ubSlider" min="-12" max="12" value="2" step="1">
        </div>
      </div>
      <div class="readout">
        <div class="readout-item highlight">
          <div class="rlabel">U_ab = U_a − U_b</div>
          <div class="rvalue" id="uabOut">6 V</div>
        </div>
      </div>
    </div>
  ...
  <section>
    ...
    <div class="panel">
      ...
      <svg class="diagram" viewBox="0 0 680 200" id="capSvg">
        <rect id="capPlatePos" x="280" y="40" width="14" height="120" fill="#c9803d"/>
        <rect id="capPlateNeg" x="386" y="40" width="14" height="120" fill="#5b8dd9"/>
      ...
      <div class="controls">
        <div class="control blue">
          <div class="control-row"><span class="label">spanning u</span><span class="value blue" id="capUVal">6 V</span></div>
          <input type="range" id="capUSlider" min="0" max="12" value="6" step="0.5">
        </div>
        <div class="control green">
          <div class="control-row"><span class="label">capaciteit C</span><span class="value green" id="capCVal">100 µF</span></div>
          <input type="range" id="capCSlider" min="10" max="300" value="100" step="10">
        </div>
      </div>
      <div class="readout">
        <div class="readout-item">
          <div class="rlabel">q = C · u</div>
          <div class="rvalue" id="capQOut">600 µC</div>
        </div>
        <div class="readout-item highlight">
          <div class="rlabel">W = ½Cu²</div>
          <div class="rvalue" id="capWOut">1.80 mJ</div>
        </div>
      </div>
    </div>
...
(function(){
  ...
  const coulombVal = document.getElementById('coulombVal');
  const coulombQOut = document.getElementById('coulombQOut');
  const coulombCounter = document.getElementById('coulombCounter');
  const electronDots = document.getElementById('electronDots');
  ...
  // ---------- 02 Delta ----------
  const deltaStartSlider = document.getElementById('deltaStartSlider');
  const deltaEndSlider = document.getElementById('deltaEndSlider');
  const deltaStartVal = document.getElementById('deltaStartVal');
  const deltaEndVal = document.getElementById('deltaEndVal');
  const deltaStart = document.getElementById('deltaStart');
  const deltaEnd = document.getElementById('deltaEnd');
  const deltaBracket = document.getElementById('deltaBracket');
  ...
  const currentQOut = document.getElementById('currentQOut');
  const currentIOut = document.getElementById('currentIOut');
  const currentCaption = document.getElementById('currentCaption');
  const chargeDots = document.getElementById('chargeDots');
  ...
  const voltageUOut = document.getElementById('voltageUOut');
  const voltageDescOut = document.getElementById('voltageDescOut');
  const reservoirWater = document.getElementById('reservoirWater');
  const voltageDropLabel = document.getElementById('voltageDropLabel');
  ...
  // ---------- 05 Ohm ----------
  const ohmUSlider = document.getElementById('ohmUSlider');
  const ohmRSlider = document.getElementById('ohmRSlider');
  const ohmUVal = document.getElementById('ohmUVal');
  const ohmRVal = document.getElementById('ohmRVal');
  const ohmIOut = document.getElementById('ohmIOut');
  const ohmNeck = document.getElementById('ohmNeck');
  const ohmFlow = document.getElementById('ohmFlow');
  const ohmGraphLine = document.getElementById('ohmGraphLine');
  const ohmGraphDot = document.getElementById('ohmGraphDot');
  ...
  // ---------- 07 Uab ----------
  const uaSlider = document.getElementById('uaSlider');
  const ubSlider = document.getElementById('ubSlider');
  const uaVal = document.getElementById('uaVal');
  const ubVal = document.getElementById('ubVal');
  const uabOut = document.getElementById('uabOut');
  const uabArrow = document.getElementById('uabArrow');
  const uabResultLabel = document.getElementById('uabResultLabel');
  ...
  // ---------- 08 Capacitor ----------
  const capUSlider = document.getElementById('capUSlider');
  const capCSlider = document.getElementById('capCSlider');
  const capUVal = document.getElementById('capUVal');
  const capCVal = document.getElementById('capCVal');
  const capQOut = document.getElementById('capQOut');
  const capWOut = document.getElementById('capWOut');
